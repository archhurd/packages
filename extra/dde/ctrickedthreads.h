 ...
 * 	(mutex_renamed_clear): Define as mutex_renamed_init instead of bogon (lost in merge).
 *
 ...
 * 	Remove mutex_renamed_special and debug_mutex_renamed.
 * 	[90/08/24            rwd]
 *
 ...
 * 	[89/11/30            rwd]
 * 	Changed mutex_renamed macros to deal with special mutex_renameds
 * 	[89/11/26            rwd]
 * 	Make mutex_renamed_{set,clear}_special routines instead of macros.
 * 	[89/11/25            rwd]
 * 	Added mutex_renamed_special to specify a need to context switch on this
 * 	mutex_renamed.
 * 	[89/11/21            rwd]
 *
 * 	Made mutex_renamed_lock a macro trying to grab the spin_lock first.
 * 	[89/11/13            rwd]
 ...
 *	(using mutex_renamed_try_lock()) in sync.c.  Changed ((char *) 0)
 ...
 *	Made spin_unlock() and mutex_renamed_unlock() into procedure calls
 ...
 *	Fixed bugs in mutex_renamed_set_name and condition_set_name
...
/* Enable mutex_renamed holder debugging */
/* #define WAIT_DEBUG */
...
// DAMNIT, linux-headers/mutex.h and /include/lock-intern.h
#undef __mutex_init
#include <spin-lock.h>
...
#define	mutex_renamed_alloc()		((mutex_renamed_t) calloc(1, sizeof(struct mutex_renamed)))
#define	mutex_renamed_init(m) \
	MACRO_BEGIN \
	...
	WAIT_CLEAR_DEBUG(m); \
	MACRO_END
#define	mutex_renamed_set_name(m, x)	((m)->name = (x))
#define	mutex_renamed_name(m)		((m)->name != 0 ? (m)->name : "?")
#define	mutex_renamed_clear(m)		mutex_renamed_init(m)
#define	mutex_renamed_free(m)		free((m))

#define mutex_renamed_try_lock(m) (spin_try_lock(&(m)->held) ? WAIT_SET_DEBUG(m), 1 : 0)
#define mutex_renamed_lock(m) \
	MACRO_BEGIN \
	if (!spin_try_lock(&(m)->held)) { \
		__mutex_renamed_lock_solid(m); \
	} \
	WAIT_SET_DEBUG(m); \
	MACRO_END
#define mutex_renamed_unlock(m) \
	MACRO_BEGIN \
	if (spin_unlock(&(m)->held), \
	    cthread_queue_head(&(m)->queue, vm_offset_t) != 0) { \
		__mutex_renamed_unlock_solid(m); \
	} \
	WAIT_CLEAR_DEBUG(m); \
	MACRO_END
/*
...
extern void	condition_wait(condition_t _cond, mutex_renamed_t _mutex_renamed);
extern int	hurd_condition_wait(condition_t _cond, mutex_renamed_t _mutex_renamed);
